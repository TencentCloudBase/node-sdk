...
class CloudBase {
    ...
    static parseContext(context) {
        ...
        let { memory_limit_in_mb, time_limit_in_ms, request_id, environ, function_version, namespace, function_name } = context;
        let parseResult = {};
        try {
            parseResult.memory_limit_in_mb = memory_limit_in_mb;
            parseResult.time_limit_in_ms = time_limit_in_ms;
            parseResult.request_id = request_id;
            parseResult.function_version = function_version;
            parseResult.namespace = namespace;
            parseResult.function_name = function_name;
    ...
    init(config = {}) {
        let { secretId, secretKey, sessionToken, env, proxy, timeout, serviceUrl, version, headers = {}, credentials, isHttp } = config;
    ...
    database(dbConfig = {}) {
        ...
        return new database_1.Db({ config: Object.assign({}, this.config, dbConfig) });
    }
    /**
     * 调用云函数
     *
