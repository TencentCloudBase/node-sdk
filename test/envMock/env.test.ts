...
describe('mock 云函数环境', () => {
  ...
  it('注入mock 云函数环境变量', async () => {
    ...
    assert(
      process.env._SCF_TCB_LOG == '1' &&
        app.logger().isSupportClsReport === false
    )
    app.logger().log({ a: 1 })
    ...
    assert(
      process.env._SCF_TCB_LOG == '1' &&
        app.logger().isSupportClsReport === true
    )
    app.logger().log({ a: 1 })
    app.logger().info({ a: 1 })
    ...
    jest.mock('../../src/utils/request.ts', () => {
      return jest.fn().mockImplementation(opts => {
        ...
        if (opts.body.action === 'database.queryDocument') {
    ...
    // secretId
    assert(
      checkRes.body.authorization.indexOf('MOCK_TENCENTCLOUD_SECRETID') >= 0
    )
    // sessionToken
