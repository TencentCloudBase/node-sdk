...
export async function callWxOpenApi(cloudbase: CloudBase, { apiName, requestData }, opts?: ICustomReqOpts ) {
  try {
 ...
 */
export async function callCompatibleWxOpenApi(cloudbase: CloudBase, { apiName, requestData }, opts?: ICustomReqOpts ) {
  const params = {
    action: 'wx.openApi',
    apiName,
  }
 ...
 * @return {Promise} 正常内容为buffer，报错为json {code:'', message:'', resquestId:''}
 */
export async function callWxPayApi(cloudbase: CloudBase, { apiName, requestData }, opts?: ICustomReqOpts) {
  const params = {
    action: 'wx.wxPayApi',
    apiName
  }
